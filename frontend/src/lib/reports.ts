...
export function openPrintablePdf(payload: ReportPayload) {
  ...
  // Compute actual time range from city trend data
  const trendPoints = payload.snapshot.cityTrend;
  let timeRangeLabel = 'CURRENT SESSION';
  if (trendPoints.length >= 2) {
    const first = new Date(trendPoints[0].minute_label).getTime();
    const last = new Date(trendPoints[trendPoints.length - 1].minute_label).getTime();
    const diffMin = Math.round((last - first) / 60000);
    if (diffMin < 60) timeRangeLabel = `LAST ${diffMin} MINUTES`;
    else if (diffMin < 1440) timeRangeLabel = `LAST ${Math.round(diffMin / 60)} HOURS`;
    else timeRangeLabel = `LAST ${Math.round(diffMin / 1440)} DAYS`;
  }
...
<div class="report-container">
  ...
  <section>
    ...
    <p style="font-size: 14px; color: #475569; background: #f1f5f9; padding: 20px; border-radius: 12px; margin: 0;">
      As of ${when}, the Riyadh environmental monitoring grid — comprising 
      <strong>${payload.onlineSensors}</strong> of <strong>${payload.totalSensors}</strong> active sensors — 
      reports a city-wide AQI of <strong>${payload.cityAqi}</strong> (${payload.cityBandLabel}). 
      The ${timeRangeLabel.toLowerCase()} trend is classified as <strong>${payload.trendLabel}</strong>. 
      ${payload.snapshot.alerts.length > 0 
        ? `<strong>${payload.snapshot.alerts.length}</strong> active alert(s) have been detected where pollutant levels exceed the configured safety threshold of ${payload.activeThreshold} µg/m³ PM2.5.` 
        : 'All monitored zones are currently operating within configured safety thresholds.'}
      ${payload.hotspots.length > 0 
        ? `The highest concentration zones are: ${payload.hotspots.slice(0, 3).map(h => h.name).join(', ')}.` 
        : ''}
    </p>
  </section>
  ...
  <footer>
    <div class="footer-left">
      Climence Environmental Intelligence Report · Generated ${when} · 
      ${payload.totalSensors} sensors deployed · ${payload.onlineSensors} online · 
      ${payload.snapshot.alerts.length} active alert(s) · Threshold: ${payload.activeThreshold} µg/m³ PM2.5
    </div>
